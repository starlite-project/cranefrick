use super::{
	ast::{
		...
		LetDef, Model, ModelType, ModelValue, Pattern, Pragma, Rule, Signature, Spec, SpecExpr,
		SpecOp, Type, TypeValue, Variant,
	},
	error::{Error, Span},
...
impl<'a> Parser<'a> {
	...
	const fn without_pos_tracking(lexer: Lexer<'a>) -> Self {
		Self {
			lexer,
			disable_pos: true,
		}
	...
	fn expect(&mut self, f: impl FnOnce(&Token) -> bool) -> Result<Token> {
		if let Some(&(pos, ref peek)) = self.lexer.peek() {
			...
			Err(self.error(self.lexer.pos(), "unexpected eof"))
		}
	}

	fn eat(&mut self, f: impl FnOnce(&Token) -> bool) -> Result<Option<Token>> {
	...
	fn is(&self, f: impl FnOnce(&Token) -> bool) -> bool {
		if let Some((.., peek)) = self.lexer.peek() {
			f(peek)
		} else {
			false
	...
	fn pos(&self) -> Pos {
		if self.disable_pos {
			...
			self.lexer
				.peek()
				.map_or_else(|| self.lexer.pos(), |(pos, ..)| *pos)
		}
	}

	fn is_lparen(&self) -> bool {
		self.is(|tok| matches!(tok, Token::LParen))
	}

	fn is_rparen(&self) -> bool {
		self.is(|tok| matches!(tok, Token::RParen))
	}

	fn is_at(&self) -> bool {
		self.is(|tok| matches!(tok, Token::At))
	}
	...
	fn is_const(&self) -> bool {
		self.is(|tok| matches!(tok, Token::Symbol(tok_s) if tok_s.starts_with('$')))
	}

	fn is_spec_bit_vector(&self) -> bool {
		self.is(
			|tok| matches!(tok, Token::Symbol(tok_s) if tok_s.starts_with("#x") || tok_s.starts_with("#b")),
		)
	}

	fn is_spec_bool(&self) -> bool {
		self.is(
			|tok| matches!(tok, Token::Symbol(tok_s) if matches!(tok_s.as_str(), "true" | "false")),
		)
	}

	fn expect_lparen(&mut self) -> Result {
		self.expect(|tok| matches!(tok, Token::LParen))
			.map(map_unit)
	}

	fn expect_rparen(&mut self) -> Result {
		self.expect(|tok| matches!(tok, Token::RParen))
			.map(map_unit)
	}

	fn expect_at(&mut self) -> Result {
		self.expect(|tok| matches!(tok, Token::At)).map(map_unit)
	}
	...
	fn eat_sym_str(&mut self, s: &str) -> Result<bool> {
		self.eat(|tok| matches!(tok, Token::Symbol(tok_s) if tok_s == s))
	...
	fn parse_def(&mut self) -> Result<Def> {
		...
		let def = match &self.expect_symbol()?[..] {
			...
			s => return Err(self.error(pos, format!("unexpected identifier: {s}"))),
		};
	...
	fn str_to_ident(&self, pos: Pos, s: &str) -> Result<Ident> {
		let first = s
			.chars()
			.next()
			.ok_or_else(|| self.error(pos, "empty symbol"))?;

		if !first.is_alphabetic() && !matches!(first, '_' | '$') {
			return Err(self.error(
				pos,
				format!("identifier '{s}' does not start with a letter or _ or $"),
			));
		}
			...
			.any(|c| !c.is_alphanumeric() && !matches!(c, '_' | '.' | '$'))
		{
			return Err(self.error(
				pos,
				format!(
					"identifier '{s}' contains invalid characters (not a-z, A-Z, 0-9, _, ., $)"
				),
			));
		}

		Ok(Ident(s.to_owned(), pos))
	}
	...
	fn parse_const(&mut self) -> Result<Ident> {
		...
		if let Some(s) = ident.0.strip_prefix('$') {
			Ok(Ident(s.to_owned(), ident.1))
		} else {
			Err(self.error(pos, "not a constant identifier; must start with a '$'"))
		}
	}
	...
	fn parse_type(&mut self) -> Result<Type> {
		...
		while self.lexer.peek().is_some_and(|(.., tok)| tok.is_sym()) {
			let sym = self.expect_symbol()?;
			if matches!(sym.as_str(), "extern") {
				is_extern = true;
			} else if matches!(sym.as_str(), "nodebug") {
				is_nodebug = true;
			} else {
				return Err(self.error(pos, format!("unknown type declaration modifier: {sym}")));
			}
		}

		let ty = self.parse_type_value()?;
		Ok(Type {
			name,
			is_extern,
			is_nodebug,
	...
	fn parse_type_value(&mut self) -> Result<TypeValue> {
		let pos = self.pos();
		self.expect_lparen()?;
		if self.eat_sym_str("primitive")? {
			...
			Err(self.error(pos, "unknown type definition"))
		}
	}
	...
	fn parse_spec_expr(&mut self) -> Result<SpecExpr> {
		...
		if self.is_spec_bit_vector() {
			let (value, width) = self.parse_spec_bit_vector()?;
			return Ok(SpecExpr::ConstBitVec { value, width, pos });
		} else if self.is_int() {
			...
			let value = self.parse_spec_bool()?;
			return Ok(SpecExpr::ConstBool { value, pos });
		} else if self.is_sym() {
			...
			if self.eat_sym_str("switch")? {
				...
				while !self.is_rparen() {
					...
					self.expect_rparen()?;
					args.push(SpecExpr::Pair { left: l, right: r });
				}
		...
		Err(self.error(pos, "unexpected spec expression"))
	}

	fn parse_spec_op(&self, s: &str) -> Result<SpecOp> {
		let pos = self.pos();
		match s {
			"=" => Ok(SpecOp::Eq),
			"and" => Ok(SpecOp::And),
	...
	fn parse_spec_bit_vector(&mut self) -> Result<(i128, i8)> {
		...
		if let Some(s) = s.strip_prefix("#b") {
			match i128::from_str_radix(s, 2) {
				...
				Err(..) => Err(self.error(pos, "not a constant binary bit vector")),
			}
		} else if let Some(s) = s.strip_prefix("#x") {
			match i128::from_str_radix(s, 16) {
				Ok(i) => Ok((i, (s.len() as i8) * 4)),
				Err(..) => Err(self.error(pos, "not a constant hex bit vector")),
			}
		} else {
			Err(self.error(
				pos,
	...
	fn parse_model(&mut self) -> Result<Model> {
		...
		let value = if self.eat_sym_str("type")? {
			...
			return Err(self.error(pos, "model must be a type or enum"));
		};
		...
		if self.eat_sym_str("Bool")? {
			...
			let width = if self.eat_sym_str("bv")? {
				if self.is_rparen() {
					...
					return Err(self.error(pos, "badly formed BitVector (bv ...)"));
				}
			} else {
				return Err(self.error(pos, "badly formed BitVector (bv ...)"));
			};
			...
			Err(self.error(pos, "model type mut be a Bool, Int, or BitVector (bv ...)"))
		}
	}
	...
	fn parse_extern(&mut self) -> Result<Extern> {
		...
		if self.eat_sym_str("constructor")? {
			...
			Err(self.error(pos, "invalid extern: must be (extern constructor ...), (extern extractor ...) or (extern const ...)"))
		}
	}
	...
	fn parse_rule(&mut self) -> Result<Rule> {
		...
		let prio = if self.is_int() {
			Some(
				i64::try_from(self.expect_int()?)
					.map_err(|error| self.error(pos, format!("invalid rule priority: {error}")))?,
			)
		} else {
			None
		};
		...
		loop {
			match self.parse_if_let_or_expr()? {
				IfLetOrExpr::IfLet(if_let) => if_lets.push(if_let),
	...
	fn parse_pattern(&mut self) -> Result<Pattern> {
		...
		if self.is_int() {
			...
			Err(self.error(pos, "unexpected pattern"))
		}
	}

	fn parse_if_let_or_expr(&mut self) -> Result<IfLetOrExpr> {
		let pos = self.pos();
		if self.is_lparen() {
			self.expect_lparen()?;
			let ret = if self.eat_sym_str("if-let")? {
				IfLetOrExpr::IfLet(self.parse_if_let()?)
			} else if self.eat_sym_str("if")? {
				self.parse_if_let_if()?.into()
			} else {
				self.parse_expr_inner_parens(pos)?.into()
			};

			self.expect_rparen()?;
			Ok(ret)
		} else {
			self.parse_expr().map(Into::into)
		}
	}

	fn parse_if_let(&mut self) -> Result<IfLet> {
		let pos = self.pos();
		let pattern = self.parse_pattern()?;
	...
	fn parse_if_let_if(&mut self) -> Result<IfLet> {
		let pos = self.pos();
		let expr = self.parse_expr()?;
		Ok(IfLet {
	...
	fn parse_expr(&mut self) -> Result<Expr> {
		...
		if self.is_lparen() {
			...
			let value = self.parse_const()?;
			Ok(Expr::ConstPrim { value, pos })
		} else if self.eat_sym_str("true")? {
			Ok(Expr::ConstBool { value: true, pos })
		} else if self.eat_sym_str("false")? {
			Ok(Expr::ConstBool { value: false, pos })
		} else if self.is_sym() {
			let name = self.parse_ident()?;
			Ok(Expr::Var { name, pos })
		} else if self.is_int() {
			let value = self.expect_int()?;
			Ok(Expr::ConstInt { value, pos })
		} else {
			Err(self.error(pos, "invalid expression"))
		}
	}
	...
	fn parse_let_def(&mut self) -> Result<LetDef> {
		let pos = self.pos();
		self.expect_lparen()?;
		let var = self.parse_ident()?;
		let ty = self.parse_ident()?;
...
impl From<IfLet> for IfLetOrExpr {
	fn from(value: IfLet) -> Self {
		Self::IfLet(value)
	}
}

impl From<Expr> for IfLetOrExpr {
...
pub fn parse(lexer: Lexer<'_>) -> Result<Vec<Def>> {
	Parser::new(lexer).parse_defs()
}

pub fn parse_without_pos(lexer: Lexer<'_>) -> Result<Vec<Def>> {
	Parser::without_pos_tracking(lexer).parse_defs()
}
