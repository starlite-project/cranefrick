...
use cranium_isle::{codegen::CodegenOptions, compile, error::Errors, lexer, parser, printer};

fn build(filename: &str) -> Result<String, Errors> {
	compile::from_files([filename], &CodegenOptions::default())
}

pub fn run_pass(filename: &str) {
	if let Err(e) = build(filename) {
...
pub fn run_fail(filename: &str) {
	match build(filename) {
		...
		Err(err) => {
			...
			eprintln!("failed, as expected:\n{err:?}");
		}
	}
}

fn build_and_link_isle(isle_filename: &str) -> (tempfile::TempDir, PathBuf) {
	let tempdir = tempfile::tempdir().unwrap();
	let code = build(isle_filename).unwrap();

	let isle_filename_base = Path::new(isle_filename)
		.file_stem()
		.unwrap()
		.to_str()
		.unwrap()
	...
	let isle_generated_code = tempdir
		.path()
		.to_path_buf()
		.join(isle_filename_base + ".rs");
	fs::write(isle_generated_code, code).unwrap();

	let rust_filename = isle_filename.replace(".isle", "") + "_main.rs";
...
pub fn run_print(isle_filename: &str) {
	// Parse.
	let original_source = fs::read_to_string(isle_filename).unwrap();
	eprintln!("original:\n{original_source}");
	...
	for (orig, rt) in iter::zip(original, round_trip) {
		assert_eq!(orig, rt);
	}
}
