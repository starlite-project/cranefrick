	...
	fn ops(&self, ops: &[BrainIr], op_count: &mut usize) -> Result<(), AssemblyError> {
		for op in ops {
			...
			match op {
				...
				BrainIr::ScaleAndMoveValueTo(options) => self.move_value_to(*options)?,
				BrainIr::ScaleAndCopyValueTo(options) => self.copy_value_to(*options)?,
				BrainIr::ScaleAndTakeValueTo(options) => self.take_value_to(*options)?,
				BrainIr::ScaleAndFetchValueFrom(options) => self.fetch_value_from(*options)?,
				BrainIr::ScaleAndReplaceValueFrom(options) => self.replace_value_from(*options)?,
